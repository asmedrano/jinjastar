...
def main(template_path, cwd_path, output_path='/tmp/rendered_templates'):
    loader = FileSystemLoader(template_path) #only jinja cares were the templates live
    env = Environment(loader=loader)
    env.filters['markdown'] = safe_markdown
    render_dir(env, template_path, cwd_path, output_path)

def render_dir(env, template_path, cwd_path, output_path):
    ...
    """
    #clean template path
    cwd_path = os.path.realpath(cwd_path)
